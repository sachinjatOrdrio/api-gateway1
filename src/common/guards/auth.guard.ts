...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
        const decoded = this.jwtService.verify(token,{secret:this.configService.get("JWT_SECRET_KEY")}); // Verify and decode the token

        const user = await this.rabbitRPC.sendRequest(
        MessagePatternEnum.FIND_USER_BY_ID,
            decoded.userId
          ).toPromise();
